...
public class Parameter
    ...
    public static Parameter create(Class concreteClass, Class declaringClass, boolean encodeByDefault,
        Class<?> rawType,
        ...
        {
            return new BeanParameter(annotations, paramAnnotation, paramName, ct.rawClass(), ct.type(), paramEncoded,
                paramDefault);
        }
        else
        {
            return new Parameter(annotations, paramAnnotation, paramSource, paramName, ct.rawClass(), ct.type(),
                paramEncoded, paramDefault);
        }
    }

    private static List<Parameter> create(Class concreteClass, Class declaringClass, boolean keepEncoded,
        Class[] parameterTypes, Type[] genericParameterTypes, Annotation[][] parameterAnnotations) {
        ...
        for (int i = 0; i < parameterTypes.length; i++)
        {
            final Parameter parameter =
                Parameter.create(concreteClass, declaringClass, keepEncoded, parameterTypes[i],
    ...
    public static List<Parameter> create(Class concreteClass, Class declaringClass, Constructor<?> ctor,
        ...
        return create(concreteClass, declaringClass, ((null != ctor.getAnnotation(Encoded.class)) || keepEncoded),
            parameterTypes, genericParameterTypes, ctor.getParameterAnnotations());
    }
    ...
    public static List<Parameter> create(Class concreteClass, Class declaringClass, Method javaMethod,
        ...
        return create(concreteClass, declaringClass, ((null != method.getAnnotation(Encoded.class)) || keepEncoded),
    ...
    public static Parameter overrideSource(Parameter original, Parameter.Source source) {
        ...
        return new Parameter(original.annotations, original.sourceAnnotation, source, source.name(),
            original.rawType,
            original.type, original.encoded, original.defaultValue);
    }
    ...
    private Parameter(Annotation[] markers, Annotation marker, Source source, String sourceName, Class<?> rawType,
    ...
    public static class BeanParameter
        ...
        private BeanParameter(final Annotation[] markers, final Annotation marker, final String sourceName,
            final Class<?> rawType, final Type type, final boolean encoded, final String defaultValue) {
            ...
            for (Field field : AccessController.doPrivileged(ReflectionHelper.getDeclaredFieldsPA(rawType)))
            {
                ...
                {
                    Parameter beanParamParameter = Parameter.create(rawType, field.getDeclaringClass(),
            ...
            for (Constructor constructor : AccessController.doPrivileged(
                ReflectionHelper.getDeclaredConstructorsPA(rawType)))
            {
    ...
    public String toString() {
        return String.format("Parameter [type=%s, source=%s, defaultValue=%s]", getRawType(), getSourceName(),
            getDefaultValue());
    }
    ...
    public boolean equals(Object o) {
        ...
        if (sourceAnnotation != null ? !sourceAnnotation.equals(parameter.sourceAnnotation)
            : parameter.sourceAnnotation != null)
        {
            return false;
        }
